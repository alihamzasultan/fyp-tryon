...
export const createOrUpdateTransaction = async (
    ...
    try {
        ...
        if (id) {
            ...
            // update wallet for nee transaction
        ...
        if (image) {
            ...
            transactionData.image = imageUploadRes.data; // Assuming uploadFileToCloudinary returns the uploaded image URL in data
        }
...
const revertAndUpdateWallets = async (
    ...
    try {
        ...
        const reverteIncomeExpenseAmount =
        ...
        if (newTransactionType == "expense") {
            ...
            if (newWallet.amount! < newTransactionAmount) {
                return {
                    success: false,
                    msg: "The selected wallet don't have enought balance"
                }
            }
        }
        ...
        await createOrUpdateWallet({
            ...
            [revertType]: reverteIncomeExpenseAmount,
...
export const fetchMonthlyStats = async (uid: string): Promise<ResponseType> => {
    try {
        ...
        twelveMonthAgo.setDate(today.getDate() - 12);
        const transactionsQuery = query(
            collection(db, "transactions"),
...
export const fetchYearlytStats = async (uid: string): Promise<ResponseType> => {
    try {
        const db = firestore;
